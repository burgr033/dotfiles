...
Register-ArgumentCompleter -CommandName ssh,scp,sftp -Native -ScriptBlock {
  ...
  function Select-SSHHost
  {
    ...
    # Run fzf interactively and return the selected host
    $selectedHost = $hosts | fzf --style full --height 40% --layout=reverse --prompt="SSH Host> " 

    if ($selectedHost)
    { [CompletionResult]::new($selectedHost, $selectedHost, 'ParameterValue', $selectedHost) 
    }
  }
